...
app.post('/deleteAccount', (req, res, next) => {
  ...
  let message = '';
  if (tel?.length !== 11)
    message += 'В номере телефона должно быть 11 символов. Включая код страны и код оператора<br>';
  ...
  db.run(
    'SELECT * FROM telephones WHERE telephone_number = ? AND delete_ = 0',
    [tel],
    (err, selectTelephonesRows) => {
      ...
      if (!selectTelephonesRows.length)
        return res.render('deleteAccount', {
          layout: 'deleteAccount',
          message: `Учетной записи, ассоциированной с телефонным номером ${tel}, не обнаружено.`,
        });
      db.run(
        'UPDATE users SET delete_ = 1 WHERE user_id = ?',
        [selectTelephonesRows[0]['user_id']],
        (err, updateUsersRows) => {
          if (err)
            return res.render('deleteAccount', {
              layout: 'deleteAccount',
              message: 'Что-то пошло не так!2',
            });
          db.run(
            'UPDATE telephones SET delete_ = 1 WHERE user_id = ?',
            [selectTelephonesRows[0]['user_id']],
            (err, updateTelephonesRows) => {
              if (err)
                return res.render('deleteAccount', {
                  layout: 'deleteAccount',
              ...
              db.run(
                'UPDATE owners SET delete_ = 1 WHERE user_id = ?',
                [selectTelephonesRows[0]['user_id']],
                (err, updateOwnersRows) => {
                  if (err)
                    return res.render('deleteAccount', {
                      layout: 'deleteAccount',
                      message: 'Что-то пошло не так!4',
                    });
                  return res.render('deleteAccount', {
                    layout: 'deleteAccount',
                    message: `Учетная запись, ассоциированная с телефонным номером ${tel}, удалена!.`,
                  });
                },
              );
            },
...
// app.post('/api/getTokens', handlers.getTokens(app))// данный запрос использовался для регистрации ТОЛЬКО по СМС больше использоваться не будет
app.post('/api/getTokens', handlers.getTokensV2(app));
...
if (require.main === module) {
  ((port) =>
    app.listen(port, '127.0.0.1', () =>
      console.log(`Express started on ${port} port ${app.get('env')} press Ctrl-C to terminate.`),
