...
const ifGetCheckGooqlePlay = require('./commonFunctions').ifGetCheckGooglePlay
...
const getFunGetSMSCodeForRegistrationByTelephone = (smsGenerator, getFunGetResponseJSON, db, ifGetCheckGooqlePlay) => {
  return app => {
    ...
    return (req, res, next) => {
      ...
      return db.run('SELECT * FROM telephones WHERE telephone_number = ?', [telephoneNumber], (err, selectTelephonesRows) => {
        ...
        if (!selectTelephonesRows.length) { // это означает, что ранее не было НИ ОДНОГО запроса для этого номера телефона, поэтому можно ничего не проверять
          return db.run('INSERT INTO users () VALUE ()', [], (err, insertUsersRows) => {
            ...
            return db.run('INSERT INTO telephones (telephone_number, user_id) VALUES (?, ?)', [telephoneNumber, userId], (err, insertTelephoneRows) => {
              ...
              if (err) {
                return res.status(200).json(getResponseJSON("0001003", 'ошибка БД INSERT INTO telephones (telephone_number, user_id) VALUES (?, ?)'))
              }
              if (telephoneNumber === fakeTel) {
                ifGetCheckGooqlePlay()
                return res.status(200).json(getResponseJSON('0010000', 'fakeTel'))
              }
              return smsGenerator(req, res, next, insertTelephoneRows.insertId, telephoneNumber, app)
            })
          })
        }
          ...
          return db.run('SELECT * FROM smscodes WHERE telephone_id = ?', [selectTelephonesRows[0].telephone_id], (err, selectSmsCodeRows) => {
            ...
            return smsGenerator(req, res, next, selectTelephonesRows[0].telephone_id, telephoneNumber, app)
          })
        }
      })
    }
...
const getFunGetTokens = (getFunGetResponseJSON, db) => {
  return app => {
    const getResponseJSON = getFunGetResponseJSON(app)
    return (req, res, next) => {
          ...
          db.run("SELECT o.owner_id AS o_owner_id , o.user_id AS o_user_id, o.comment_ AS o_comment_ , o.time_ AS o_time_, o.delete_ AS o_delete_, u.user_name AS u_user_name, u.comment_ AS u_comment_ , u.time_ AS u_time_, u.delete_ AS u_delete_ FROM owners o INNER JOIN users u ON o.user_id = u.user_id WHERE o.user_id = ?", [userId], (err, selectOwnersRows) => {
            ...
            if (!selectOwnersRows.length) {
              db.run('INSERT INTO owners (user_id) VALUES (?)', [userId], (err, insertOwnersRows) => {
                ...
                return makeConnectionUpdateUsersReturnJSON(req, res, next, db, getResponseJSON, addTextInComment, getTokenFunction, userId, credentials, accessTokenExpire, refreshTokenExpire, userName, { id: insertOwnersRows.insertId, toDeleteOwner: true }, 'reg')
              })
            } else {
              return makeConnectionUpdateUsersReturnJSON(req, res, next, db, getResponseJSON, addTextInComment, getTokenFunction, userId, credentials, accessTokenExpire, refreshTokenExpire, userName ? userName : selectOwnersRows[0].u_user_name, { id: selectOwnersRows[0].o_owner_id, toDeleteOwner: false }, 'auth')
            }
          })
        })
      })
...
exports.getSMSCodeFun = getFunGetSMSCodeForRegistrationByTelephone(smsGenerator, getFunGetResponseJSON, db, ifGetCheckGooqlePlay)
exports.getFunGetSMSCodeForRegistrationByTelephone = getFunGetSMSCodeForRegistrationByTelephone
exports.getTokens = getFunGetTokens(getFunGetResponseJSON, db)
